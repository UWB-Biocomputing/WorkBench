...
public class ScriptManager {
     ...
     *                           simulated spatial information (or names of files which contain
     *                           such spatial data). This is the file that is constructed using the
    ...
    public static Script generateScript(String projectName, String version,
        ...
        if (simulatorSourceCodeUpdatingType != null) {
            updateRepo = simulatorSourceCodeUpdatingType.equals(SimulationSpecification.GIT_PULL_AND_CLONE);
        }
        /* Create Script */
        ...
        if (simSpec.isRemote()) {
            ...
            script.addVerbatimStatement("mkdir -p " + "workbenchconfigfiles" + folderDelimiter + "NList", null, true);
        } else {
            String[] mkResultsDirArgs = {userDir + simFolder + folderDelimiter + "results"};
            script.executeProgram("mkdir", mkResultsDirArgs);
            script.addVerbatimStatement("mkdir -p " + userDir + simFolder + folderDelimiter + "workbenchconfigfiles"
                    + folderDelimiter + "NList", null, true);
        }
     ...
     *         stored. Depending on the form of the project folder provided, this may represent
     *         a relative path.
     */
     ...
     */
    public boolean runScript(ProvMgr provMgr, SimulationSpecification simSpec, String scriptPath, String scriptVersion,
            String[] nListFilenames, String simConfigFilename) throws JSchException, FileNotFoundException,
            SftpException {
        boolean success;
    ...
    private boolean runRemoteScript(ProvMgr provMgr, SimulationSpecification simSpec, String scriptPath,
            String scriptVersion, String[] nListFilenames, String simConfigFilename) throws JSchException,
            ...
            if (sft.uploadFile(scriptPath, "", hostname, lcd.getUsername(), password, null)) {
                ...
                if (provMgr != null) {
                    ...
                    WorkbenchOperationRecorder.uploadFile(provMgr, scriptPath, "~/"
                            + FileManager.getSimpleFilename(scriptPath), "script", simSpec.getHostAddr(),
                            "uploadScript_v" + scriptVersion, uploadStartTime, new Date());
                ...
                if (nListFilenames != null) {
                    for (int i = 0, im = nListFilenames.length; i < im; i++) {
                        ...
                        loopSuccess = sft.uploadFile(nListFilenames[i], "", hostname, lcd.getUsername(), password,
                                null);
                        if (!loopSuccess) {
                            success = false;
                            ...
                            if (provMgr != null) {
                                ...
                                String NLType = "";
                                try {
                                    NLType = InputAnalyzer.getInputType(new File(nListFilenames[i])).toString();
                                } catch (ParserConfigurationException | SAXException | IOException ex) {
                                }
                                WorkbenchOperationRecorder.uploadFile(provMgr, nListFilenames[i],
                                        "~/" + FileManager.getSimpleFilename(nListFilenames[i]), "nlist",
                                        simSpec.getHostAddr(), "upload_" + NLType + "_NList_" + "for_Script_v"
                ...
                if (success) {
                    uploadStartTime = new Date();
                    success = sft.uploadFile(simConfigFilename, "", hostname, lcd.getUsername(), password, null);
                    if (success) {
                        if (provMgr != null) {
                            Long startTime = System.currentTimeMillis();
        ...
        if (provMgr != null) {
            DateTime.recordAccumulatedProvTiming("ScriptManager", "runRemoteScript", accumulatedTime);
        }
        return success;
    }

    private boolean runLocalScript(ProvMgr provMgr, SimulationSpecification simSpec, String scriptLocation,
        ...
        Path scriptTargetPath = Paths.get(fm.getUserDir() + FileManager.getSimpleFilename(scriptLocation));
        Path simConfigSourcePath = Paths.get(simConfigFilename);
        Path simConfigTargetPath = Paths.get(fm.getUserDir() + FileManager.getSimpleFilename(simConfigFilename));
        ...
        if (inputFilenames != null && inputFilenames.length > 0) {
            ...
            for (int i = 0, im = inputFilenames.length; i < im; i++) {
                nListTargetPaths[i] = Paths.get(fm.getUserDir() + FileManager.getSimpleFilename(inputFilenames[i]));
            }
        }
        try {
            ...
            if (provMgr != null) {
                Long startTime = System.currentTimeMillis();
                WorkbenchOperationRecorder.copyFile(provMgr, scriptSourcePath.toString(), scriptTargetPath.toString(),
                        "script", "copy_Script_v" + scriptVersion, copyStartTime, new Date());
                accumulatedTime = DateTime.sumProvTiming(startTime, accumulatedTime);
            }
            outstandingMessages += "\nFile copied..."
        ...
        String cmd = "sh " + fm.getUserDir() + FileManager.getSimpleFilename(scriptTargetPath.toString());
        // run the script
        try {
        ...
        DateTime.recordFunctionExecutionTime("ScriptManager", "runLocalScript",
                System.currentTimeMillis() - functionStartTime, provMgr != null);
        if (provMgr != null) {
            DateTime.recordAccumulatedProvTiming("ScriptManager", "runLocalScript", accumulatedTime);
        }
        if (success) {
     ...
     *
     * Note: This class defines the context between provenance data. The provenance manager is
     * used to connect such data as determined by this function. This means that if script
     * generation changes this function may need to change, in turn, and vice-versa.
     *
     ...
     * @param projectMgr
     * @param provMgr  Provenance manager used to create provenance based on analysis of the
     *                 printf output
     ...
     */
    public long analyzeScriptOutput(SimulationSpecification simSpec, ProjectMgr projectMgr, ProvMgr provMgr,
        ...
        String localOutputFilename = fetchScriptOutputFiles(projectMgr, simSpec, outputTargetFolder);
        if (localOutputFilename != null) {
            ...
            if (timeCompleted != DateTime.ERROR_TIME && provMgr != null) {
                ...
                if (sim != null && simSuccessful) {
                    ...
                    Resource simAgent = provMgr.addSoftwareAgent(uri, "simulator", simSpec.isRemote(), false);
                    // get activity resource
                    Resource simActivity = provMgr.addActivity("simulation_" + UUID.randomUUID(), "simulation",
                            simSpec.isRemote(), false);
                    // connect the two
                    ...
                    String remoteSimConfigFilename = userDir + simSpec.getSimulatorFolder() + "/workbenchconfigfiles/"
                    ...
                    Resource remoteSimConfigFileEnitity = provMgr.addEntity(remoteSimConfigFilename,
                            "simulationConfigurationFile", simSpec.getHostAddr(), simSpec.getUsername(),
                            "sftp", false);
                    provMgr.atLocation(remoteSimConfigFileEnitity, location);

                    provMgr.used(simActivity,remoteSimConfigFileEnitity);
                    String[] neuronLists = FileManager.getFileManager().getNeuronListFilenames(projectMgr.getName());
                    for (int i = 0, im = neuronLists.length; i < im; i++) {
                        ...
                        Resource movedNLFileEntity = provMgr.addEntity(movedNLFilename, "nlist", simSpec.getHostAddr(),
                        ...
                        try { // try to start reading from the given file path
                            ...
                            if (fileReader.hasNext()) {
                                ...
                                if (!sha1key.contains("fatal")) {
                                    provMgr.wasDerivedFrom(simAgent, provMgr.addEntity(simSpec.getCodeLocation()
                                            .substring(8, simSpec.getCodeLocation().lastIndexOf("."))
                                            + "/commit/" + sha1key, "commit", null, null, "https", false));
                                }
                            }
...
//                }
            ...
            DateTime.recordFunctionExecutionTime("ScriptManager", "analyzeScriptOutput",
                    System.currentTimeMillis() - functionStartTime, provMgr != null);
            if (provMgr != null) {
                DateTime.recordAccumulatedProvTiming("ScriptManager", "analyzeScriptOutput", accumulatedTime);
            }
        }
    ...
    private String fetchScriptOutputFiles(ProjectMgr projectMgr, SimulationSpecification simSpec,
        ...
        if (remote) {
            ...
            outstandingMessages += "\nDownloading script status file:\n" + scriptStatusFilename
                    + "\nFrom: " + hostname + "\n";
            if (sft.downloadFile(scriptStatusFilename, scriptStatusFileTargetLocation, hostname, lcd.getUsername(),
                    password)) {
                ...
                try {
                    ...
                    sft.downloadFile(simStatusFilename, localSimResultsFolder + simStatusFilename, hostname,
                ...
                if (scriptComplete(scriptStatusFileTargetLocation, simSpec)) {
                    ...
                    outstandingMessages += "\nDownloading simulator source code version report:\n"
                            ...
                            + "\nFrom: " + hostname + "\n";
                    try {
                        sft.downloadFile(SHA1KeyFilename, outputStorageFolder + SHA1KeyFilename, hostname,
                                lcd.getUsername(), password);
                    } catch (SftpException e) {
                        outstandingMessages += "\nDownload failed for: "
                                + SHA1KeyFilename
    ...
    private boolean scriptComplete(String localScriptOutputFilename, SimulationSpecification simSpec) {
        long timeCompleted;
        OutputAnalyzer analyzer = new OutputAnalyzer();
     ...
     *
     * @return The messages that have accumulated since the last call to this function in the
    ...
    private static void printfSimSpecToScript(Script script, String simFile, String simInputFilename, boolean append) {
        script.printf(SimulationSpecification.SIM_EXEC_TEXT, simFile, null, append);
        String joinedInputs = simInputFilename;
        script.printf(SimulationSpecification.SIM_INPUTS_TEXT, joinedInputs, null, true);
        // printf the outputs
        script.printf(SimulationSpecification.SIM_OUTPUTS_TEXT, "output.xml", null, true);
        // printf the end tag for the sim spec data
        script.printf(SimulationSpecification.END_SIM_SPEC_TEXT, "", null, true);
    }
